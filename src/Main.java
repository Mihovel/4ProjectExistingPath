...
public class Main {
    ...
    static int[][] arr = new int[COLUMNS][COLUMNS];
    ...
    public static void recursionPart(List<TwoIndexes> twoIndexesList, TwoIndexes currentPoint, int maxLengthJump) {
        ...
        if ((currentPoint.getI() != 6) && (currentPoint.getJ() != 6)) {
            ...
            if (!suitablePoints.isEmpty()) { isPathExists(twoIndexesList, suitablePoints, maxLengthJump); } else { System.out.println("No such a path"); }
